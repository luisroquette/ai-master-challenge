# Manifesto dos entregáveis do Challenge 001

## Regra de publicação

Nenhum item externo será apresentado como pronto antes de abrir sem autenticação e passar pela revisão factual. O Markdown e os artefatos canônicos permanecem suficientes para auditar a resposta se uma mídia externa ficar indisponível.

| Entrada | Artefato | Estado | Validação exigida |
|---|---|---|---|
| Leia em 2 minutos | `delivery-brief.md` | Pronto localmente | Limite de palavras, fatos e links locais |
| Leia em 2 minutos | `delivery-brief.docx` | Pronto localmente | Duas páginas A4 inspecionadas integralmente |
| Veja funcionando em 90 segundos | Arcade | Roteiro de oito passos pronto; não publicado | Preview factual e acesso anônimo |
| Entenda a arquitetura do trabalho em 6 minutos | Vídeo obrigatório | Metadados, poster e capítulos prontos; não publicado | Reprodução integral em tempo real e acesso anônimo |
| Veja a síntese | NotebookLM Video Overview e infográfico | Fonte e prompts prontos; outputs não gerados | Revisão factual e acesso anônimo |
| Audite o processo | Relatório, código e Process Log | Pronto localmente | Setup reproduzível e links válidos |

## Verdades que não podem divergir

1. Churn recente de 12,4%, alta de 7,0 pontos percentuais.
2. MRR perdido observado de US$ 1.622.337, sem promessa de recuperação.
3. Uso agregado sobe; uso na coorte de churn em 30 dias cai.
4. Satisfação cobre respondentes de tickets, não toda a base.
5. Nenhuma causa ou concentração material foi demonstrada.
6. A decisão é validar dados, uso e satisfação antes de intervir.

## Bloqueios ativos

- Push, hospedagem externa e PR aguardam todos os gates verdes.
- Os quatro desafios devem chegar à branch única `submission/luis-roquette` e a um único PR.
- O diff final deve permanecer dentro de `submissions/luis-roquette/`.
